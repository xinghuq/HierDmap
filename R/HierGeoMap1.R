...
ggDivgeomap=function(data,x=Longitude, y= Latitude,scale = "medium", returnclass = "sf",size=HiereHGDPq1$HierD$MDpop,title="Global Genetic Diversity Map",subtitle=("Scaled relative genetic diversity of HGDP populations"),...){
###get world map data
...
rDivgeomap=function(x=Longitude, y=Latitude, xlim = c(-180, 180), ylim = c(-90, 90), 
                    asp = 1, bg = "aliceblue", cols = "white", fill = T, border = "lightgrey", 
                   wrap=c(-180,180),col = rgb(red = 0, green = 0, blue = 1, alpha = 0.3),
                    # define size as number of flights div. by 50
                    size =  normalize(HiereHGDPq1$HierD$MDpop)*10, pch = 20,...){
# get map
...
plot(worldmap, xlim=xlim, ylim = ylim, 
     asp = asp, bg = bg, col = cols, fill = fill, border = border, 
      wrap=wrap,...)
points(x, y,col =col,cex =  size, pch=pch,... )
...
dDivgeomap=function(data,x=Longitude, y=Latitude,label=Population,border_colour=NA, border_fill="antiquewhite",density_alpha = I(.2),
  ...
  ggplot2::ggplot(data, (aes(x, y))) +   
    ggplot2:: borders("world", colour=border_colour, fill=border_fill)  +
    ggplot2::stat_density_2d(data = data,aes(x, y, fill = density_fill,  alpha = I(.2)),
...
#library(ggmap)
# define box
dgDivgeomap=function(data,x = Longitude, y= Latitude,lon = c(-160, 160), lat = c(-60, 70),  mapsource = "osm", mapcolor = "color", maptype="satellite",
                    point_color="red", pointalpha = .2, pointsize=normalize(HiereHGDPq1$HierD$MDpop)*10,...){
